 ...
 * 
 * Creates a new test account and returns authentication token for Flow 2 testing.
 * 
...
function getOTPFromUser() {
  return new Promise((resolve) => {
    ...
    rl.question('Enter the OTP sent to your email (or check MongoDB OTP collection): ', (otp) => {
      rl.close();
      resolve(otp.trim());
    });
  });
...
async function requestSignupOTP() {
  ...
  if (response.success) {
    ...
    console.log('   Note: OTP is sent via email (or check MongoDB OTP collection)');
    return true;
  } else {
 ...
 */
async function main() {
  console.log('🚀 Creating Test Account for Flow 2 Testing');
  ...
  try {
    ...
    if (otpRequested) {
      ...
      console.log('\n📝 Please check your email or MongoDB for the OTP.');

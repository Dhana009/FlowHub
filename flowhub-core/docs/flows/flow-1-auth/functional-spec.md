...
**Flow ID:** FLOW-001  
**PRD Reference:** prd.md (Version 2.1)
...
- **On Click:**
  ...
  3. If all fields valid:
     ...
     - Request body: `{ "email": "user@example.com", "password": "password123", "rememberMe": false }`
...
- **On Success (200 OK):**
  - Response: `{ "token": "jwt-token", "refreshToken": "refresh-token", "user": {...} }`
...
2. **OTP Generation:**
   ...
   - System stores OTP in database (with expiration: 10 minutes)
   - System shows OTP input field
   - System shows message: "OTP has been generated. Please check database for OTP."
...
4. **Account Creation:**
   ...
   - Request body: `{ "firstName": "John", "lastName": "Doe", "email": "user@example.com", "password": "Password123!", "otp": "123456" }`
...
1. **OTP Request:**
   ...
   - **If exists:** Generate OTP, store in database, show generic message
   - **If doesn't exist:** Show same generic message (security - doesn't reveal)
   - System shows OTP input field (only if email exists)
...
3. **Password Reset:**
   ...
   - System validates: Password meets strength requirements, passwords match
...
**AC-3.2:** User can reset password with valid OTP
...
**AC-3.3:** New password strength validation works
...
### **7.3 Account Lockouts Table/Collection (Optional)**
...
- Database (SQLite/MongoDB) for user data, OTPs

      ...
      <div className="h-full px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
        <div className="flex items-center justify-between h-full">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <nav className="flex items-center gap-2 text-sm font-medium" aria-label="Breadcrumb navigation">
              <button
              ...
              <div className="flex items-center h-6 text-slate-400" aria-hidden="true">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </div>
              <span className="text-slate-900 font-bold leading-6 h-6 flex items-center" aria-current="page">
          ...
          <nav className="flex items-center gap-3" aria-label="User account utility navigation">
            {/* User Role Badge - Desktop Prominent */}
            <div className="hidden sm:flex items-center mr-2">
              <span 
                className={`px-3 py-1 rounded-lg text-[10px] font-bold border tracking-wider uppercase shadow-sm ${
                  user?.role === 'ADMIN' ? 'bg-purple-50 text-purple-700 border-purple-200' :
                  user?.role === 'EDITOR' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                  'bg-slate-50 text-slate-600 border-slate-200'
                }`}
                aria-label={`Current role: ${user?.role} Access`}
              >
                {user?.role} Access
              </span>
            </div>
            ...
            <div className="relative">
              ...
              >
                {/* Avatar */}
                <div className="w-9 h-9 bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-xl flex items-center justify-center text-white text-sm font-bold shadow-sm" aria-hidden="true">
              ...
              {userMenuOpen && (
                <>
                  ...
                  >
                    <div className="px-4 py-3 border-b border-slate-100 md:hidden" role="none">
                      <p className="text-sm font-semibold text-slate-900">
                        {user?.firstName} {user?.lastName}
                      </p>
                      <div className="flex items-center gap-2 mt-0.5" role="none">
                    ...
                    <div className="border-t border-slate-100 my-1" role="separator" aria-hidden="true" />
                    <button
